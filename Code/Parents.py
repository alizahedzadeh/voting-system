...
class Parents:
    ...
    def login(self):
        ...
        if self.logged_in:
            ...
            res = Dbase.login("parents", user, password)

            if res[0]:
                self.logged_in = True
                self.parents_id = res[1][0]
    ...
    def polls_for_parents(self):

        if self.logged_in:
            Dbase = Database()
            result = Dbase.show_polls_students_and_parents(("parents", None), "audience")

            if len(result) == 0:
                print("No polls were created for parents")
                return True
    ...
    def answer_poll(self):
        if self.logged_in:
            if self.polls_for_parents():
                ...
                answered_polls = Dbase.check_answered_poll("parents", self.parents_id)[0]
                ...
                if not (answered_polls is None) and (_id in answered_polls):
                    ...
                    if result is None:
                        ...
                        Dbase.update_answered_poll("parents", self.parents_id, self.poll_id)
            else:
